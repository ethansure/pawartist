...
const styles = [
  { id: "vangogh", nameKey: "vanGogh", emoji: "🌻" },
  { id: "monet", nameKey: "monet", emoji: "🌸" },
  { id: "picasso", nameKey: "picasso", emoji: "🎭" },
  { id: "anime", nameKey: "anime", emoji: "🌸" },
  { id: "sketch", nameKey: "sketch", emoji: "✏️" },
  { id: "watercolor", nameKey: "watercolor", emoji: "💧" },
  { id: "oilpainting", nameKey: "oilPainting", emoji: "🖼️" },
  { id: "cyberpunk", nameKey: "cyberpunk", emoji: "🌆" },
];
        ...
        <section className="pt-32 pb-20 px-6">
          <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
              <p className="text-sm text-rose-400 mb-3 text-center">✨ {t("styles.vanGogh")}</p>
              <div className="flex items-center gap-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {styles.map((style, i) => (
                <div key={i} className="text-center bg-white/5 rounded-2xl p-4 border border-white/10 hover:border-rose-500/30 transition">
                  ...
                  <div className="font-medium text-sm">{t(`styles.${style.nameKey}`)}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="max-w-4xl mx-auto">
                ...
                <div className="grid grid-cols-3 md:grid-cols-4 gap-3">
                  {styles.map(style => (
                    <button key={style.id} onClick={() => setSelectedStyle(style.id)} className={`p-3 rounded-xl text-center transition-all ${selectedStyle === style.id ? "bg-rose-500 text-white" : "bg-white/5 hover:bg-white/10 border border-white/10"}`}>
                      ...
                      <div className="text-xs">{t(`styles.${style.nameKey}`)}</div>
                    </button>
                  ))}
                </div>
              </div>
          ...
          {processing && (
            <div className="text-center py-16">
              ...
              <h2 className="text-2xl font-bold mb-4">{t("applying")}</h2>
